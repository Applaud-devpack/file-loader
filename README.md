...
The `@applaud/file-loader` resolves `import`/`require()` on a file into a url and emits the file into the output directory.

## Getting Started

To begin, you'll need to install `@applaud/file-loader`:

```console
$ npm install @applaud/file-loader --save-dev
```
...
The following examples show how one might use `@applaud/file-loader` and what the result would be.
